...
for (var i = 0; i < operator2.length; i++) {
    operator2[i].addEventListener("click", function(e) {
      ...
      var currentString = input.innerHTML;
      var lastChar = currentString[currentString.length - 1];
      if (lastChar === "+" || lastChar === "-" || lastChar === "×" || lastChar === "÷" || currentString.length == 0) {
        input.innerHTML += e.target.innerHTML;
      }
      else{
    ...
    //problème avec le signe ) impossible à mettre a la fin...
  }
...
result.addEventListener("click", function() {
      ...
      var newcalc = [];
      for (var i = Dcalculsec+1; i <Fcalculsec; i++) {
          newcalc.push((input.innerHTML)[i]);
      }

      var numbers2 = newcalc.split(/\+|\-|\×|\÷/g);//var numbers = (input.innerHTML).split(/\+|\-|\(|\)|\×|\÷/g); avec les parenthèses
      var operators2 = newcalc.replace(/[0-9]|\./g, "").split("");
    
      var divide = operators2.indexOf("÷");
      while (divide != -1) {
        numbers2.splice(divide, 2, numbers2[divide] / numbers2[divide + 1]);
      ...
      while (multiply != -1) {
        numbers2.splice(multiply, 2, numbers2[multiply] * numbers2[multiply + 1]);
      ...
      while (subtract != -1) {
        numbers2.splice(subtract, 2, numbers2[subtract] - numbers2[subtract + 1]);
      ...
      while (add != -1) {
        ...
        numbers2.splice(add, 2, parseFloat(numbers2[add]) + parseFloat(numbers2[add + 1]));
        operators2.splice(add, 1);
        add = operators2.indexOf("+");
      }

      //problème je ne comprend pas comment retirer le calcul du reste de mon tableau.
  }*/
